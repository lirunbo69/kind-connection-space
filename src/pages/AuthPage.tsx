...
const DashboardMockup = () => (
  <div className="relative w-full max-w-lg animate-[fadeSlideUp_1s_ease-out_0.3s_both]">
      ...
      >
        ...
        <svg viewBox="0 0 300 80" className="w-full h-16">
          ...
          {[[0, 60], [90, 40], [180, 25], [270, 15]].map(([cx, cy], i) => (
            <circle key={i} cx={cx} cy={cy} r="3" fill="hsl(36 90% 52%)"
    ...
    <div
      className="absolute -top-4 -right-4 glass-strong rounded-xl shadow-lg p-3 flex items-center gap-2"
    ...
    >
      ...
      <div>
        <div className="text-[10px] font-medium text-muted-foreground">转化提升</div>
        <div className="text-xs font-bold text-primary">+32%</div>
      </div>
    </div>

    <div
      className="absolute -bottom-3 -left-3 glass-strong rounded-xl shadow-lg px-3 py-2 flex items-center gap-2"
      style={{ animation: "floatBounce 4s ease-in-out 1s infinite, fadeSlideUp 0.6s ease-out 2s both" }}
    >
      <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-amber-400 to-primary flex items-center justify-center">
        <Zap className="w-3 h-3 text-primary-foreground" />
      </div>
      <span className="text-[10px] font-medium text-muted-foreground">AI智能生成中...</span>
    </div>
  </div>
);

const HeroPanel = () => (
  <div className="hidden lg:flex flex-col justify-center items-center flex-1 relative overflow-hidden bg-mesh-auth p-12">
    <div className="absolute inset-0 overflow-hidden">
      <div className="absolute w-[500px] h-[500px] rounded-full bg-purple-200/15 -top-32 -left-32 animate-[pulseScale_8s_ease-in-out_infinite]" />
      <div className="absolute w-[400px] h-[400px] rounded-full bg-blue-200/12 -bottom-20 -right-20 animate-[pulseScale_10s_ease-in-out_2s_infinite]" />
      <div className="absolute w-[200px] h-[200px] rounded-full bg-amber-200/10 top-1/4 right-1/4 animate-[pulseScale_6s_ease-in-out_1s_infinite]" />
    ...
    <div className="relative z-10 max-w-xl flex flex-col items-center text-center">
      ...
      <div className="grid grid-cols-2 gap-3 mt-8 w-full max-w-md">
        {features.map((f, i) => (
          <div
            key={f.title}
            className="flex items-start gap-2.5 p-3 rounded-xl glass hover-lift transition-all"
...
const FormLayout = ({ children }: { children: React.ReactNode }) => (
  <div className="min-h-screen flex">
    <HeroPanel />
    <div className="flex-1 flex items-center justify-center p-6 bg-mesh relative">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-primary/60 via-amber-400 to-primary/60 lg:hidden" />
      <div className="w-full max-w-sm space-y-6">
        <div className="flex flex-col items-center gap-2 lg:hidden mb-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary to-amber-600 flex items-center justify-center shadow-lg shadow-primary/30">
            <span className="text-primary-foreground font-bold text-2xl">Mt</span>
          </div>
          <h1 className="text-xl font-bold text-foreground">Mt ListingAI</h1>
        </div>
        {children}
      </div>
    </div>
...
const AuthPage = ({ onAuth, defaultTab = "login", onBack }: { onAuth: () => void; defaultTab?: string; onBack?: () => void }) => {
  ...
  const btnClass = "w-full bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl h-11 shadow-lg shadow-primary/20 transition-all";
  ...
  if (view === "forgot-password") {
    return (
      <FormLayout>
        ...
        <Card className="glass-strong rounded-2xl border-0 shadow-xl animate-[glassReveal_0.5s_ease-out_0.1s_both]">
      ...
      <Card className="glass-strong rounded-2xl border-0 shadow-xl animate-[glassReveal_0.5s_ease-out_0.1s_both]">
        <CardContent className="pt-6">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-2 mb-4 glass rounded-xl h-11">
              <TabsTrigger value="login" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md transition-all">登录</TabsTrigger>
              <TabsTrigger value="signup" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md transition-all">注册</TabsTrigger>
            </TabsList>

            <TabsContent value="login">
              <Tabs defaultValue="phone-pwd">
                <TabsList className="grid w-full grid-cols-2 mb-4 h-9 glass rounded-lg">
                  <TabsTrigger value="phone-pwd" className="text-xs gap-1 rounded-md data-[state=active]:bg-primary/15 data-[state=active]:text-primary">
                    <Phone className="w-3 h-3" />手机号登录
                  </TabsTrigger>
                  <TabsTrigger value="email-pwd" className="text-xs gap-1 rounded-md data-[state=active]:bg-primary/15 data-[state=active]:text-primary">
                ...
                <TabsContent value="phone-pwd">
                  <form onSubmit={handlePhoneLogin} className="space-y-4">
                    <div className="space-y-2">
                      <Label>手机号</Label>
                      <Input type="tel" placeholder="+86 13800138000" value={phone} onChange={(e) => setPhone(e.target.value)} required className={inputClass} />
                    </div>
                    <div className="space-y-2">
                      <Label>密码</Label>
            ...
            <TabsContent value="signup">
              <Tabs defaultValue="phone-signup">
                <TabsList className="grid w-full grid-cols-2 mb-4 h-9 glass rounded-lg">
                  <TabsTrigger value="phone-signup" className="text-xs gap-1 rounded-md data-[state=active]:bg-primary/15 data-[state=active]:text-primary">
                    <Phone className="w-3 h-3" />手机号注册
                  </TabsTrigger>
                  <TabsTrigger value="email-signup" className="text-xs gap-1 rounded-md data-[state=active]:bg-primary/15 data-[state=active]:text-primary">
                ...
                <TabsContent value="phone-signup">
                  <form onSubmit={handlePhoneSignup} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label>密码</Label>
                      <Input type="password" placeholder="至少6位密码" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className={inputClass} />
                    </div>
                    <Button type="submit" className={btnClass} disabled={loading}>
                      {loading ? "注册中..." : "注册"}
                    </Button>
